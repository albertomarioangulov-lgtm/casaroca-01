...
const { fetchCaregivers } = useCaregivers()
...
const initializeViewMode = () => {
  ...
  try {
    stored = localStorage.getItem('children-view-mode')
  } catch {}
...
const toggleView = () => {
  ...
  try {
    localStorage.setItem('children-view-mode', viewMode.value)
  } catch {}
}

const handleEdit = (child: Record<string, any>) => {
  openEdit(child)
}
...
onMounted(() => {
  ...
  window.addEventListener('resize', () => {
    try {
      const stored = localStorage.getItem('children-view-mode')
      if (!stored) {
        viewMode.value = window.innerWidth < 768 ? 'cards' : 'table'
      }
    } catch {}
...
<template>
  <template v-if="can(PERMISSIONS.CHILDREN_READ)">
    ...
    <ChildrenForm @saved="handleSaved" />
